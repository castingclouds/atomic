...
where
    ...
    F: Fn(Vertex<NodeId>, Vertex<NodeId>) -> bool,
    K: FnMut(&Hash) -> bool,
{
...
fn collect_nondeleted_zombies<T, K>(
    ...
    graph: &mut T::Graph,
    mut known: K,
    source: Vertex<NodeId>,
    target: Vertex<NodeId>,
    zombies: &mut Vec<NodeId>,
) -> Result<(), LocalApplyError<T>>
where
    T: GraphMutTxnT + TreeTxnT,
    K: FnMut(&Hash) -> bool,
...
pub(crate) fn find_source_vertex<T: GraphMutTxnT + TreeTxnT>(
    ...
    flag: EdgeFlags,
    ws: &mut super::Workspace,
) -> Result<Vertex<NodeId>, LocalApplyError<T>> {
    ...
    for &p in ws.parents.iter() {
        ...
        for &c in ws.children.iter() {
            if p != c {
                ...
                put_graph_with_rev(txn, channel, EdgeFlags::PSEUDO, p, c, NodeId::ROOT)?;
            }
        }
    }
    Ok(())
...
fn zombify<T: GraphMutTxnT>(
    ...
    inode: Position<Option<Hash>>,
    flag: EdgeFlags,
    unknown: &[(Vertex<NodeId>, SerializedEdge)],
) -> Result<(), MissingError<T::GraphError>> {
