...
pub enum UnrecordError<ChangestoreError: std::error::Error + 'static, T: GraphTxnT + TreeTxnT> {
    ...
    ChangeNotInChannel { hash: NodeId },
...
#[derive(Default)]
struct Workspace {
    up: HashMap<Vertex<NodeId>, Position<Option<Hash>>>,
    down: HashMap<Vertex<NodeId>, Position<Option<Hash>>>,
    parents: HashSet<Vertex<NodeId>>,
    del: Vec<SerializedEdge>,
    apply: crate::apply::Workspace,
    stack: Vec<Vertex<NodeId>>,
    del_edges: Vec<(Vertex<NodeId>, SerializedEdge)>,
    must_reintroduce: HashSet<(Vertex<NodeId>, Vertex<NodeId>)>,
    zombies_stack: Vec<(Vertex<NodeId>, bool, bool)>,
}
