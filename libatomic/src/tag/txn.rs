...
impl GraphTxnT for TagTxn {
    ...
    fn get_graph<'txn>(
        ...
        unsafe {
            ...
            let gr: Db<Vertex<NodeId>, SerializedEdge> = Db::from_page(db.graph);
    ...
    fn get_external<'txn>(
        ...
        unsafe {
            ...
            let gr: UDb<NodeId, SerializedHash> = UDb::from_page(self.header.offsets.external);
    ...
    fn get_internal<'txn>(
        &'txn self,
        int: &SerializedHash,
    ) -> Result<Option<&'txn NodeId>, TxnErr<Self::GraphError>> {
        unsafe {
            use crate::pristine::sanakirja::UDb;
            let gr: UDb<SerializedHash, NodeId> = UDb::from_page(self.header.offsets.internal);
            Ok(::sanakirja::btree::get(self, &gr, int, None)?.map(|(_, v)| v))
        }
    }

    fn get_node_type(&self, _id: &NodeId) -> Result<Option<NodeType>, TxnErr<Self::GraphError>> {
        // TagTxn doesn't have access to node_types table
        Ok(None)
    }
    ...
    fn init_adj(
        ...
        unsafe {
            ...
            use crate::pristine::sanakirja::Db;
            let gr: Db<Vertex<NodeId>, SerializedEdge> = Db::from_page(g.graph);
    ...
    fn find_block(
        ...
        unsafe {
            use crate::pristine::sanakirja::Db;
            let gr: Db<Vertex<NodeId>, SerializedEdge> = Db::from_page(graph.graph);
...
impl ChannelTxnT for TagTxn {
    ...
    fn get_changeset(
        ...
        unsafe {
            use crate::pristine::sanakirja::Db;
            let db: Db<NodeId, L64> = Db::from_page(*channel);
    ...
    fn get_revchangeset(
        ...
        unsafe {
            use crate::pristine::sanakirja::UDb;
            let db: UDb<L64, Pair<NodeId, SerializedMerkle>> = UDb::from_page(*revchanges);
            match ::sanakirja::btree::get(self, &db, c, None) {
                Ok(Some((k, x))) if k == c => Ok(Some(x)),
                Ok(_) => Ok(None),
                Err(e) => {
    ...
    fn cursor_changeset<'a>(
        ...
        channel: &Self::Changeset,
        pos: Option<NodeId>,
    ) -> Result<Cursor<Self, &'a Self, Self::ChangesetCursor, NodeId, L64>, TxnErr<TagError>>
    {
        unsafe {
            use crate::pristine::sanakirja::Db;
            let db: Db<NodeId, L64> = Db::from_page(*channel);
    ...
    > {
        unsafe {
            use crate::pristine::sanakirja::UDb;
            let db: UDb<L64, Pair<NodeId, SerializedMerkle>> = UDb::from_page(*channel);
    ...
    ) -> Result<Option<(&L64, &Pair<NodeId, SerializedMerkle>)>, TxnErr<TagError>> {
    ...
    ) -> Result<Option<(&NodeId, &L64)>, TxnErr<TagError>> {
        if let Ok(x) = cursor.next(self) {
            Ok(x)
        } else {
            Err(TxnErr(SanakirjaError::PristineCorrupt).into())
    ...
    type TagsCursor = ::sanakirja::btree::cursor::Cursor<
        L64,
        crate::pristine::TagBytes,
        ::sanakirja::btree::page_unsized::Page<L64, crate::pristine::TagBytes>,
    >;

    fn is_tagged(&self, tags: &Self::Tags, t: u64) -> Result<bool, TxnErr<Self::GraphError>> {
        unsafe {
            use crate::pristine::sanakirja::UDb;
            let db: UDb<L64, crate::pristine::TagBytes> = UDb::from_page(*tags);
            let t: L64 = t.into();
    ...
    fn cursor_tags<'txn>(
        ...
        crate::pristine::Cursor<Self, &'txn Self, Self::TagsCursor, L64, crate::pristine::TagBytes>,
        TxnErr<Self::GraphError>,
    > {
        unsafe {
            use crate::pristine::sanakirja::UDb;
            let db: UDb<L64, crate::pristine::TagBytes> = UDb::from_page(*channel);
            let mut cursor = ::sanakirja::btree::cursor::Cursor::new(self, &db)?;
            if let Some(k) = k {
                cursor.set(self, &k, None)?;
            }
    ...
    ) -> Result<Option<(&L64, &crate::pristine::TagBytes)>, TxnErr<Self::GraphError>> {
        if let Ok(x) = cursor.next(self) {
            Ok(x)
        } else {
            Err(TxnErr(SanakirjaError::PristineCorrupt).into())
    ...
    fn iter_tags(
        ...
        crate::pristine::Cursor<Self, &Self, Self::TagsCursor, L64, crate::pristine::TagBytes>,
        TxnErr<Self::GraphError>,
    > {
    ...
    fn rev_iter_tags(
        ...
        crate::pristine::RevCursor<Self, &Self, Self::TagsCursor, L64, crate::pristine::TagBytes>,
        TxnErr<Self::GraphError>,
    > {
        unsafe {
            use crate::pristine::sanakirja::UDb;
...
impl<T: GraphTxnT> GraphTxnT for WithTag<T> {
    ...
    fn get_node_type(&self, _id: &NodeId) -> Result<Option<NodeType>, TxnErr<Self::GraphError>> {
        // WithTag doesn't have access to node_types table
        Ok(None)
    }
...
impl<T: GraphTxnT> WithTag<T> {
    pub fn channel(&self) -> ChannelRef<Self> {
        ChannelRef::new(self.tag.header.offsets.clone())
    }
}

impl<T: GraphTxnT> ChannelTxnT for WithTag<T> {
...
impl<T: TreeTxnT> TreeTxnT for WithTag<T> {
    ...
    fn get_inodes(
        &self,
        a: &Inode,
        b: Option<&Position<NodeId>>,
    ) -> Result<Option<&Position<NodeId>>, TreeErr<Self::TreeError>> {
    ...
    fn cursor_inodes<'txn>(
        &'txn self,
        a: &Self::Inodes,
        b: Option<(&Inode, Option<&Position<NodeId>>)>,
    ) -> Result<
        crate::pristine::Cursor<Self, &'txn Self, Self::InodesCursor, Inode, Position<NodeId>>,
        TreeErr<Self::TreeError>,
    > {
    ...
    fn cursor_revinodes<'txn>(
        &'txn self,
        a: &Self::Revinodes,
        b: Option<(&Position<NodeId>, Option<&Inode>)>,
    ) -> Result<
        crate::pristine::Cursor<Self, &'txn Self, Self::RevinodesCursor, Position<NodeId>, Inode>,
        TreeErr<Self::TreeError>,
    > {
    ...
    ) -> Result<Option<(&Inode, &Position<NodeId>)>, TreeErr<Self::TreeError>> {
    ...
    ) -> Result<Option<(&Position<NodeId>, &Inode)>, TreeErr<Self::TreeError>> {
    ...
    fn cursor_partials<'txn>(
        &'txn self,
        a: &Self::Partials,
        b: Option<(&SmallStr, Option<&Position<NodeId>>)>,
    ) -> Result<
        crate::pristine::Cursor<
            Self,
            &'txn Self,
    ...
    ) -> Result<Option<(&SmallStr, &Position<NodeId>)>, TreeErr<Self::TreeError>> {
