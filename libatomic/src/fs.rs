...
//! tracked by atomic (i.e. adding files, removing files, getting file
//! names…).
//!
//! atomic tracks files in two different ways: one is the *graph*,
...
pub fn get_vertex<T: TreeTxnT>(
    txn: &T,
    path: &str,
) -> Result<(bool, Option<Position<NodeId>>), TreeErr<T::TreeError>> {
    debug!("is_tracked {:?}", path);
    let (inode, mut remaining_path_components) = closest_in_repo_ancestor(txn, path)?;
    debug!("/is_tracked {:?}", path);
...
pub fn working_copy_children<'txn, T: TreeTxnT>(
    txn: &'txn T,
    inode: Inode,
) -> Result<WorkingCopyChildren<'txn, T>, T::TreeError> {
...
pub fn iter_working_copy<'txn, T: TreeTxnT>(
    txn: &'txn T,
    root: Inode,
) -> WorkingCopyIterator<'txn, T> {
    WorkingCopyIterator {
...
impl<'txn, T: GraphTxnT> Iterator for GraphDescendants<'txn, T> {
    type Item = Result<Position<NodeId>, T::GraphError>;
    fn next(&mut self) -> Option<Self::Item> {
...
{
    type Item = Result<(Position<NodeId>, NodeId, InodeMetadata, String), T::GraphError>;
    fn next(&mut self) -> Option<Self::Item> {
        let dest = loop {
...
{
    type Item = Result<(Position<NodeId>, InodeMetadata, String), T::GraphError>;
    fn next(&mut self) -> Option<Self::Item> {
        loop {
...
pub fn iter_paths<T: GraphTxnT, F: FnMut(&mut dyn Iterator<Item = Position<NodeId>>) -> bool>(
    txn: &T,
    graph: &T::Graph,
    key: Position<NodeId>,
    mut f: F,
) -> Result<(), TxnErr<T::GraphError>> {
    let mut stack: Vec<(Position<NodeId>, bool)> = vec![(key, true)];
...
pub(crate) fn follow_oldest_path<T: ChannelTxnT, C: ChangeStore>(
    ...
    channel: &T::Channel,
    path: &str,
) -> Result<(Position<NodeId>, bool), FsErrorC<C::Error, T>> {
    use crate::pristine::*;
...
pub fn get_latest_touch<'a, T: ChannelTxnT + DepsTxnT<DepsError = <T as GraphTxnT>::GraphError>>(
    ...
    channel: &T::Channel,
    pos: &Position<NodeId>,
) -> Result<(u64, NodeId), TxnErr<T::GraphError>> {
