...
pub struct Builder {
    ...
    recorded_inodes: Arc<Mutex<HashMap<Inode, Position<Option<NodeId>>>>>,
    deleted_vertices: Arc<Mutex<HashSet<Position<NodeId>>>>,
    pub force_rediff: bool,
    pub ignore_missing: bool,
    pub contents: Arc<Mutex<Vec<u8>>>,
    new_root: Arc<Mutex<Option<(Position<Option<NodeId>>, u64)>>>,
}
...
pub struct Recorded {
    ...
    pub actions: Vec<Hunk<Option<NodeId>, LocalByte>>,
...
fn get_inodes_<T: ChannelTxnT + TreeTxnT, C: ChangeStore, W: WorkingCopyRead>(
    ...
    inode: &Inode,
) -> Result<Option<Position<NodeId>>, RecordError<C::Error, W::Error, T>> {
    let txn = txn.read();
...
fn get_inodes<'a, T: ChannelTxnT + TreeTxnT, C: ChangeStore, W: WorkingCopyRead>(
    ...
    channel: &T::Channel,
    inode: &Inode,
) -> Result<Option<&'a Position<NodeId>>, RecordError<C::Error, W::Error, T>> {
...
impl Builder {
        ...
        <W as WorkingCopyRead>::Error: 'static,
    {
        ...
        while let Some((mut item, mut components)) = stack.pop() {
            ...
            let vertex: Option<Position<Option<NodeId>>> =
...
impl Recorded {
    fn add_root_if_needed(
        ...
        if v_papa.change == Some(NodeId::ROOT) {
    ...
    fn add_file<W: WorkingCopyRead>(
        ...
        working_copy: &W,
        item: RecordItem,
    ) -> Result<Option<Position<Option<NodeId>>>, W::Error> {
    ...
    fn record_existing_file<T: ChannelTxnT + TreeTxnT, W: WorkingCopyRead + Clone, C: ChangeStore>(
        ...
        changes: &C,
        item: &RecordItem,
        new_papa: Option<Position<Option<NodeId>>>,
        vertex: Position<NodeId>,
    ) -> Result<(), RecordError<C::Error, W::Error, T>>
    where
        <W as crate::working_copy::WorkingCopyRead>::Error: 'static,
    {
    ...
    fn record_moved_file<T: ChannelTxnT + TreeTxnT, C: ChangeStore, W: WorkingCopyRead>(
        ...
        item: &RecordItem,
        vertex: Position<NodeId>,
        new_papa: Position<Option<NodeId>>,
        encoding: Option<Encoding>,
    ) -> Result<(), RecordError<C::Error, W::Error, T>>
    where
        <W as crate::working_copy::WorkingCopyRead>::Error: 'static,
...
#[derive(Debug)]
struct MovedEdges {
    edges: Vec<NewEdge<Option<NodeId>>>,
    alive: Vec<NewEdge<Option<NodeId>>>,
    resurrect: Vec<NewEdge<Option<NodeId>>>,
    need_new_name: bool,
    n_alive_names: usize,
}
...
fn collect_moved_edges<T: GraphTxnT + TreeTxnT, C: ChangeStore, W: WorkingCopyRead>(
    ...
    changes: &C,
    channel: &T::Graph,
    parent_pos: Position<Option<NodeId>>,
    current_pos: Position<NodeId>,
    new_meta: InodeMetadata,
    name: &str,
    ...
    for parent in iter_adjacent(
        ...
        for grandparent in iter_adjacent(
            ...
            *parent_dest,
            ...
            let grandparent_changed = if parent_pos.change == Some(NodeId::ROOT) {
...
impl Recorded {
    fn record_deleted_file<T: GraphTxnT + TreeTxnT, W: WorkingCopyRead, C: ChangeStore>(
        ...
        working_copy: &W,
        full_path: &str,
        current_vertex: Position<NodeId>,
        changes: &C,
    ) -> Result<(), RecordError<C::Error, W::Error, T>>
    where
        <W as WorkingCopyRead>::Error: 'static,
