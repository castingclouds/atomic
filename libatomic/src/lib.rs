...
pub use crate::apply::{apply_change_arc, apply_node, apply_node_ws, ApplyError, LocalApplyError};
...
pub use crate::attribution::{
    AIMetadata, AttributedPatch, AttributedPatchFactory, AttributionError, AttributionStats,
    AuthorId, AuthorInfo, PatchId, SuggestionType,
};
...
pub use crate::pristine::{
    get_header_by_hash, ArcTxn, Base32, ChannelMutTxnT, ChannelRef, ChannelTxnT, DepsTxnT,
    EdgeFlags, GraphTxnT, Hash, Inode, Merkle, MutTxnT, NodeId, OwnedPathId, RemoteRef, TreeTxnT,
    TxnT, Vertex,
};
...
pub trait MutTxnTExt:
    pristine::MutTxnT
    + pristine::TagMetadataMutTxnT<TagError = <Self as pristine::GraphTxnT>::GraphError>
{
    ...
    // Unified node application methods (Phase 5)
    fn apply_node<C: changestore::ChangeStore>(
        &mut self,
        changes: &C,
        channel: &mut Self::Channel,
        hash: &pristine::Hash,
        node_type: pristine::NodeType,
    ) -> Result<(u64, pristine::Merkle), crate::apply::ApplyError<C::Error, Self>> {
        crate::apply::apply_node(changes, self, channel, hash, node_type)
    }

    fn apply_node_ws<C: changestore::ChangeStore>(
        &mut self,
        changes: &C,
        channel: &mut Self::Channel,
        hash: &pristine::Hash,
        ...
        crate::apply::apply_node_ws(changes, self, channel, hash, node_type, workspace)
    }
    ...
    fn apply_node_rec<C: changestore::ChangeStore>(
        ...
        crate::apply::apply_node_rec(changes, self, channel, hash, node_type)
    }
    ...
    fn apply_node_rec_ws<C: changestore::ChangeStore>(
        ...
        crate::apply::apply_node_rec_ws(changes, self, channel, hash, node_type, workspace, false)
    }
...
pub trait TxnTExt: pristine::TxnT {
    ...
    fn is_alive(
        ...
        a: &pristine::Vertex<pristine::NodeId>,
    ...
    fn log_for_path<'channel, 'txn>(
        &'txn self,
        channel: &'channel Self::Channel,
        pos: pristine::Position<pristine::NodeId>,
        from: u64,
    ...
    fn changeid_reverse_log<'txn>(
        ...
        pristine::RevCursor<
            ...
            pristine::L64,
            pristine::Pair<pristine::NodeId, pristine::SerializedMerkle>,
        >,
        Self::GraphError,
    > {
    ...
    fn touched_files<'txn>(
        &'txn self,
        h: &pristine::Hash,
    ) -> Result<Option<Touched<'txn, Self>>, Self::DepsError> {
    ...
    fn follow_oldest_path<C: changestore::ChangeStore>(
        ...
        channel: &pristine::ChannelRef<Self>,
        path: &str,
    ) -> Result<(pristine::Position<pristine::NodeId>, bool), fs::FsErrorC<C::Error, Self>> {
...
impl<
        T: MutTxnT + pristine::TagMetadataMutTxnT<TagError = <T as pristine::GraphTxnT>::GraphError>,
    > ArcTxn<T>
{
...
pub struct Touched<'txn, T: pristine::DepsTxnT> {
    ...
    iter: pristine::Cursor<
        ...
        pristine::NodeId,
        pristine::Position<pristine::NodeId>,
    >,
    id: pristine::NodeId,
}
