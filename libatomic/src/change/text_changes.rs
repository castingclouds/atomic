    ...
    pub fn write<W: WriteChangeLine, C: ChangeStore>(
        ...
        fn change_message<C: ChangeStore>(changes: &C, change: &Hash) -> String {
            match changes.get_header(change) {
                ...
                Err(e) => {
                    // Check if this might be a tag (file not found error)
                    let error_string = e.to_string();
                    if error_string.contains("No such file or directory")
                        || error_string.contains("not found")
                    {
                        // Likely a tag - show with tag icon
                        format!("🏷️  Tag")
                    } else {
                        format!("[couldn't get change description: {}]", e)
                    }
                }
            }
        }
        ...
        // Write transitive dependencies from hunks, but stop at tag boundaries
        // Tags act as "frontiers" that prevent unbounded transitive dependency growth
        //
        ...
        // - Reduce redundant dependencies in text format (cleaner, smaller files)
        // - Skip listing changes already covered by tag dependencies
        // - Require thread-safe tag metadata access during serialization
        //
        // Currently disabled because:
        // - Known to cause deadlocks during push operations
        // - Needs architectural changes to pass change store safely
        // - Not critical: tag consolidation works fine in the graph without this
        //
        // See: docs/PHASE-5-VERIFICATION-COMPLETE.md for dependency consolidation details
        let tag_metadata_cache: std::collections::HashMap<Hash, Vec<Hash>> =
            std::collections::HashMap::new();

        self.write_all_deps(|change| {
            // Skip if this change is already in dependencies (including tags)
            if self.dependencies.contains(&change) {
                return Ok(());
            }

            // Check if this change is consolidated by a tag in our dependencies
            let mut is_consolidated = false;
            for consolidated_changes in tag_metadata_cache.values() {
                if consolidated_changes.contains(&change) {
            ...
            if let Entry::Vacant(e) = hashes.entry(change) {
                // Only write the dependency line if it's not consolidated by a tag
                if !is_consolidated {
                    e.insert(i);
                    if !needs_newline {
                        w.write_all(Self::DEPS_LINE.as_bytes())?;
                    ...
                    // For consolidated changes, use a placeholder index
                    // They're in hashes map for position lookup but not written
                    e.insert(0);
                }
            }
            Ok(())
...
impl Change {
    ...
    >(
        ...
        let (mut deps, extra) =
            dependencies(txn, &channel.read(), change.hashed.changes.iter(), false)?;
...
pub fn to_printable_pos(
    ...
    let change = match pos.change {
        Some(ref c) if c.is_none() => 1,
        Some(ref c) => *hashes.get(c).unwrap(),
        None => 0,
    };
