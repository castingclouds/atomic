...
fn hash_mismatch(change: &mut Change) -> Result<(), anyhow::Error> {
    ...
    let h = change.serialize(&mut buf, |_, _| Ok::<_, anyhow::Error>(()))?;
    // Create a corrupted hash by modifying the Merkle point
    let corrupted_h = {
        let mut hasher = crate::pristine::Hasher::default();
        hasher.update(b"corrupted");
        hasher.finish()
    };
    match Change::deserialize(buf.path().to_str().unwrap(), Some(&corrupted_h)) {
