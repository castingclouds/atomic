...
fn hash_mismatch(patch: &Patch3) -> Result<(), anyhow::Error> {
    ...
    let h = patch.serialize(&mut buf)?;
    // Create a corrupted hash by generating a different Ed25519 point
    let corrupted_h = {
        let mut hasher = crate::pristine::Hasher::default();
        hasher.update(b"corrupted_patch");
        hasher.finish()
    };
    match Patch3::deserialize(buf.path().to_str().unwrap(), &corrupted_h) {
        Err(e) => {
            let e = e.downcast();
