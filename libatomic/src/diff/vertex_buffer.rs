...
pub(super) struct Diff {
    pub inode: Position<Option<NodeId>>,
    pub path: String,
...
pub struct Vertex {
    pub pos: usize,
    pub vertex: crate::pristine::Vertex<NodeId>,
...
impl Diff {
    ...
    pub fn position(&self, i: usize, pos: usize) -> crate::pristine::Position<NodeId> {
...
impl vertex_buffer::VertexBuffer for Diff {
    fn output_line<E, C>(&mut self, v: crate::pristine::Vertex<NodeId>, c: C) -> Result<(), E>
    where
        E: From<std::io::Error>,
