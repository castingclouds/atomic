use crate::pristine::{NodeId, SerializedEdge, Vertex};
...
impl AliveVertex {
    ...
    pub fn new(vertex: Vertex<NodeId>) -> Self {
        AliveVertex {
            vertex,
            flags: Flags::empty(),
            children: 0,
...
pub(crate) fn remove_redundant_children(
    graph: &Graph,
    vids: &HashMap<Vertex<NodeId>, crate::alive::VertexId>,
    vertices: &mut HashSet<Vertex<NodeId>>,
    target: Vertex<NodeId>,
) {
...
pub(crate) fn remove_redundant_parents(
    ...
    vids: &HashMap<Vertex<NodeId>, crate::alive::VertexId>,
    vertices: &mut HashSet<Vertex<NodeId>>,
    covered: &mut HashSet<(Vertex<NodeId>, Vertex<NodeId>)>,
    target: Vertex<NodeId>,
) {
    let mut min = std::usize::MAX;
    let mut stack = Vec::new();
