...
/// Node type discriminator for the dependency graph.
///
/// All nodes in the dependency DAG have a structural type that determines
/// how they behave and what data they contain. Changes have hunks and apply
/// to the working copy, while tags consolidate history without modifying files.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, Serialize, Deserialize)]
#[repr(u8)]
pub enum NodeType {
    /// Regular change with hunks that modifies files
    Change = 0,
    /// Consolidating tag that represents a historical snapshot
    Tag = 1,
}

impl NodeType {
    /// Convert a u8 value to a NodeType.
    ///
    /// Returns None if the value doesn't correspond to a valid node type.
...
pub trait GraphTxnT: Sized {
    ...
    table!(graph);
    get!(graph, Vertex<NodeId>, SerializedEdge, GraphError);
    /// Returns the external hash of an internal change identifier, if
    /// the change is known.
    fn get_external(&self, p: &NodeId)
    ...
    fn get_internal(&self, p: &SerializedHash)
        -> Result<Option<&NodeId>, TxnErr<Self::GraphError>>;

    /// Returns the node type for an internal ID.
    ///
    /// Returns `None` if the internal ID is not found in the node types table.
    ...
    fn find_block<'a>(
        ...
        graph: &Self::Graph,
        p: Position<NodeId>,
    ) -> Result<&'a Vertex<NodeId>, BlockError<Self::GraphError>>;
...
pub trait ChannelTxnT: GraphTxnT {
    ...
    fn cursor_changeset<'txn>(
        ...
        crate::pristine::Cursor<Self, &'txn Self, Self::ChangesetCursor, NodeId, L64>,
        TxnErr<Self::GraphError>,
    >;
    fn cursor_changeset_next(
        &self,
        cursor: &mut Self::ChangesetCursor,
    ) -> Result<Option<(&NodeId, &L64)>, TxnErr<Self::GraphError>>;
    ...
    fn rev_cursor_revchangeset<'txn>(
        ...
        RevCursor<Self, &'txn Self, Self::RevchangesetCursor, L64, Pair<NodeId, SerializedMerkle>>,
        TxnErr<Self::GraphError>,
    >;
    fn cursor_revchangeset_next(
        &self,
        cursor: &mut Self::RevchangesetCursor,
    ) -> Result<Option<(&L64, &Pair<NodeId, SerializedMerkle>)>, TxnErr<Self::GraphError>>;
    ...
    fn iter_tags(
        ...
        channel: &Self::Tags,
        from: u64,
    ) -> Result<Cursor<Self, &Self, Self::TagsCursor, L64, TagBytes>, TxnErr<Self::GraphError>>;

    fn rev_iter_tags(
        &self,
        channel: &Self::Tags,
        from: Option<u64>,
    ) -> Result<RevCursor<Self, &Self, Self::TagsCursor, L64, TagBytes>, TxnErr<Self::GraphError>>;
}
...
impl<'a, T: GraphIter> Iterator for GraphIterator<'a, T> {
    type Item = Result<(&'a Vertex<NodeId>, &'a SerializedEdge), TxnErr<T::GraphError>>;
...
pub trait DepsTxnT: Sized {
    ...
    table!(revdep);
    table!(dep);
    table_get!(dep, NodeId, NodeId, DepsError);
    cursor_ref!(dep, NodeId, NodeId, DepsError);
    table_get!(revdep, NodeId, NodeId, DepsError);
    fn iter_revdep(
        &self,
        p: &NodeId,
    ) -> Result<Cursor<Self, &Self, Self::DepCursor, NodeId, NodeId>, TxnErr<Self::DepsError>>;
    fn iter_dep(
        &self,
        p: &NodeId,
    ) -> Result<Cursor<Self, &Self, Self::DepCursor, NodeId, NodeId>, TxnErr<Self::DepsError>>;
    fn iter_dep_ref<RT: std::ops::Deref<Target = Self> + Clone>(
        txn: RT,
        p: &NodeId,
    ) -> Result<Cursor<Self, RT, Self::DepCursor, NodeId, NodeId>, TxnErr<Self::DepsError>>;
    fn iter_touched(
        &self,
        p: &Position<NodeId>,
    ) -> Result<
        Cursor<Self, &Self, Self::Touched_filesCursor, Position<NodeId>, NodeId>,
    ...
    fn iter_rev_touched(
        ...
        Cursor<Self, &Self, Self::Rev_touched_filesCursor, NodeId, Position<NodeId>>,
        TxnErr<Self::DepsError>,
    >;
    table!(touched_files);
    table!(rev_touched_files);
    table_get!(touched_files, Position<NodeId>, NodeId, DepsError);
    table_get!(rev_touched_files, NodeId, Position<NodeId>, DepsError);
    iter!(touched_files, Position<NodeId>, NodeId, DepsError);
    iter!(rev_touched_files, NodeId, Position<NodeId>, DepsError);
}

/// Trait for reading consolidating tag data from the database.
///
/// This trait provides read-only access to consolidating tags and their
/// attribution summaries. Tags are stored with Hash as the key.
///
/// # Increment 2 Implementation
///
/// Currently uses in-memory HashMap storage for API validation.
/// Increment 3 will add proper Sanakirja btree persistence.
pub trait TagMetadataTxnT: Sized {
    type TagError: std::error::Error + Send + Sync + 'static;

    /// Get a consolidating tag by its hash.
    ///
    /// Returns the serialized tag data if it exists.
    fn get_tag(&self, hash: &Hash) -> Result<Option<SerializedTag>, TxnErr<Self::TagError>>;

    /// Get a tag attribution summary by tag hash.
    ///
    /// Returns the serialized attribution summary if it exists.
    fn get_tag_attribution_summary(
        &self,
        hash: &Hash,
    ) -> Result<Option<SerializedTagAttributionSummary>, TxnErr<Self::TagError>>;

    /// Check if a consolidating tag exists.
    fn has_tag(&self, hash: &Hash) -> Result<bool, TxnErr<Self::TagError>>;

    /// Get the latest semantic version tag from a channel.
    ///
    /// Iterates through all consolidating tags in the channel and returns
    /// the one with the highest semantic version.
    ///
    /// # Arguments
    ///
    /// * `channel` - The channel name to search in
    ///
    /// # Returns
    ///
    /// The latest version string if any tags with versions exist, None otherwise.
    fn get_latest_version(&self, channel: &str) -> Result<Option<String>, TxnErr<Self::TagError>> {
        // Default implementation returns None
        // Implementations should override this to query tags from the database
        let _ = channel;
        Ok(None)
    }

    /// Generate the next patch version based on the latest tag in the channel.
    ///
    /// This is the DEFAULT version increment behavior.
    /// Queries the latest semantic version tag and increments the patch number.
    /// If no version exists, returns "0.0.1" as the initial version.
    ///
    /// # Arguments
    ///
    /// * `channel` - The channel name to search in
    ...
    /// # Returns
    ///
    /// The next patch version string (e.g., "1.0.1" if latest is "1.0.0")
    fn get_next_patch_version(&self, channel: &str) -> Result<String, TxnErr<Self::TagError>> {
        use crate::pristine::tag::SemanticVersion;

        if let Some(latest) = self.get_latest_version(channel)? {
            match SemanticVersion::parse(&latest) {
                Ok(version) => Ok(version.increment_patch().to_string()),
                Err(_) => {
                    // If we can't parse the version, default to 0.0.1
                    Ok("0.0.1".to_string())
                }
            }
        } else {
            // No version found, start with 0.0.1
            Ok("0.0.1".to_string())
        }
    }

    /// Generate the next minor version based on the latest tag in the channel.
    ///
    /// Use with --minor flag to increment the minor version instead of patch.
    ///
    /// # Returns
    ///
    /// The next minor version string (e.g., "1.1.0" if latest is "1.0.5")
    fn get_next_minor_version(&self, channel: &str) -> Result<String, TxnErr<Self::TagError>> {
        use crate::pristine::tag::SemanticVersion;

        if let Some(latest) = self.get_latest_version(channel)? {
            match SemanticVersion::parse(&latest) {
                Ok(version) => Ok(version.increment_minor().to_string()),
                Err(_) => Ok("0.0.1".to_string()),
            }
        } else {
            Ok("0.0.1".to_string())
        }
    ...
    /// Generate the next major version based on the latest tag in the channel.
    ///
    /// Use with --major flag to increment the major version instead of patch.
    ///
    /// # Returns
    ///
    /// The next major version string (e.g., "2.0.0" if latest is "1.5.3")
    fn get_next_major_version(&self, channel: &str) -> Result<String, TxnErr<Self::TagError>> {
        use crate::pristine::tag::SemanticVersion;

        if let Some(latest) = self.get_latest_version(channel)? {
            match SemanticVersion::parse(&latest) {
                Ok(version) => Ok(version.increment_major().to_string()),
                Err(_) => Ok("0.0.1".to_string()),
            }
        } else {
            Ok("0.0.1".to_string())
...
pub trait TreeTxnT: Sized {
    ...
    table!(inodes);
    table!(revinodes);
    table_get!(inodes, Inode, Position<NodeId>, TreeError, TreeErr);
    table_get!(revinodes, Position<NodeId>, Inode, TreeError, TreeErr);

    table!(partials);
    cursor!(partials, SmallStr, Position<NodeId>, TreeError, TreeErr);
    cursor!(inodes, Inode, Position<NodeId>, TreeError, TreeErr);
    fn iter_inodes(
        &self,
    ) -> Result<
        Cursor<Self, &Self, Self::InodesCursor, Inode, Position<NodeId>>,
        TreeErr<Self::TreeError>,
    >;

    // #[cfg(debug_assertions)]
    cursor!(revinodes, Position<NodeId>, Inode, TreeError, TreeErr);
    // #[cfg(debug_assertions)]
    fn iter_revinodes(
        &self,
    ) -> Result<
        Cursor<Self, &Self, Self::RevinodesCursor, Position<NodeId>, Inode>,
        TreeErr<Self::TreeError>,
    >;
    fn iter_partials<'txn>(
        ...
        Cursor<Self, &'txn Self, Self::PartialsCursor, SmallStr, Position<NodeId>>,
        TreeErr<Self::TreeError>,
    >;
}
    ...
    + TreeTxnT<TreeError = <Self as GraphTxnT>::GraphError>
{
    ...
    fn hash_from_prefix(
        &self,
        prefix: &str,
    ) -> Result<(Hash, NodeId), HashPrefixError<Self::GraphError>>;
    ...
    ///
    /// Returns None if the hash is not found or node type is not registered.
    /// Following AGENTS.md node-type-aware architecture.
    fn get_node_type_by_hash(&self, hash: &Hash) -> Option<NodeType> {
        // Map Hash -> NodeId -> NodeType
        let shash = hash.into();
...
pub(crate) fn internal<T: GraphTxnT>(
    ...
    h: &Option<Hash>,
    p: NodeId,
) -> Result<Option<NodeId>, TxnErr<T::GraphError>> {
    match h {
        Some(ref hash) if hash.is_none() => Ok(Some(NodeId::ROOT)),
...
pub fn internal_pos<T: GraphTxnT>(
    ...
    change_id: NodeId,
) -> Result<Position<NodeId>, InconsistentChange<T::GraphError>> {
...
pub fn internal_vertex<T: GraphTxnT>(
    ...
    change_id: NodeId,
) -> Result<Vertex<NodeId>, InconsistentChange<T::GraphError>> {
...
pub fn changeid_log<'db, 'txn: 'db, T: ChannelTxnT>(
    ...
    Cursor<T, &'txn T, T::RevchangesetCursor, L64, Pair<NodeId, SerializedMerkle>>,
    TxnErr<T::GraphError>,
> {
...
pub(crate) fn changeid_rev_log<'db, 'txn: 'db, T: ChannelTxnT>(
    ...
    RevCursor<T, &'txn T, T::RevchangesetCursor, L64, Pair<NodeId, SerializedMerkle>>,
    TxnErr<T::GraphError>,
> {
...
pub fn debug_dep<T: DepsTxnT>(txn: &T) {
    debug!("debug_dep");
    for t in txn.iter_dep(&NodeId::ROOT).unwrap() {
...
pub fn debug_revdep<T: DepsTxnT>(txn: &T) {
    debug!("debug_revdep");
    for t in txn.iter_revdep(&NodeId::ROOT).unwrap() {
...
pub fn check_alive<T: ChannelTxnT + GraphIter>(
    ...
    HashMap<Vertex<NodeId>, Option<Vertex<NodeId>>>,
    Vec<(Vertex<NodeId>, Option<Vertex<NodeId>>)>,
) {
...
fn find_file<T: GraphTxnT>(
    ...
    channel: &T::Graph,
    k: Vertex<NodeId>,
    stack: &mut Vec<Vertex<NodeId>>,
    visited: &mut HashSet<Vertex<NodeId>>,
) -> Result<Option<Vertex<NodeId>>, TxnErr<T::GraphError>> {
    let mut file = None;
    stack.clear();
    stack.push(k);
    visited.clear();
...
fn debug_vertex<W: std::io::Write>(mut f: W, k: Vertex<NodeId>) -> Result<(), std::io::Error> {
    writeln!(
        f,
        "node_{}_{}_{}[label=\"{} [{};{}[\"];",
        k.change.to_base32(),
...
initialized_cursor!(changeset, NodeId, L64, ChannelTxnT, GraphError);
initialized_cursor!(
    revchangeset,
    L64,
...
initialized_cursor!(tags, L64, TagBytes, ChannelTxnT, GraphError);
initialized_rev_cursor!(tags, L64, TagBytes, ChannelTxnT, GraphError);
...
initialized_cursor!(dep, NodeId, NodeId, DepsTxnT, DepsError);
initialized_cursor!(
    partials,
    SmallStr,
    Position<NodeId>,
...
initialized_cursor!(touched_files, Position<NodeId>, NodeId, DepsTxnT, DepsError);
...
pub struct PathChangeset<'channel, 'txn: 'channel, T: ChannelTxnT + DepsTxnT> {
    txn: &'txn T,
    channel: &'channel T::Channel,
    iter: Cursor<T, &'txn T, T::RevchangesetCursor, L64, Pair<NodeId, SerializedMerkle>>,
    key: Position<NodeId>,
}
...
pub struct RevPathChangeset<'channel, 'txn: 'channel, T: ChannelTxnT + DepsTxnT> {
    txn: &'txn T,
    channel: &'channel T::Channel,
    iter: RevCursor<T, &'txn T, T::RevchangesetCursor, L64, Pair<NodeId, SerializedMerkle>>,
    key: Position<NodeId>,
}
...
pub trait GraphMutTxnT: GraphTxnT {
    put_del!(internal, SerializedHash, NodeId, GraphError);
    put_del!(external, NodeId, SerializedHash, GraphError);
    ...
    /// Delete the node type entry for an internal ID.
    ///
...
pub trait DepsMutTxnT: DepsTxnT {
    put_del!(dep, NodeId, NodeId, DepsError);
    put_del!(revdep, NodeId, NodeId, DepsError);
    put_del!(touched_files, Position<NodeId>, NodeId, DepsError);
    put_del!(rev_touched_files, NodeId, Position<NodeId>, DepsError);
}

/// Trait for writing consolidating tag data to the database.
///
/// This trait provides mutable access to consolidating tags and their
...
/// Currently uses in-memory HashMap storage for API validation.
/// Increment 3 will add proper Sanakirja btree persistence.
pub trait TagMetadataMutTxnT: TagMetadataTxnT {
    /// Store a consolidating tag.
    ///
    /// Overwrites any existing tag with the same hash.
    fn put_tag(&mut self, hash: &Hash, tag: &SerializedTag) -> Result<(), TxnErr<Self::TagError>>;

    /// Store a tag attribution summary.
    ///
    /// Overwrites any existing summary with the same hash.
...
pub(crate) fn del_graph_with_rev<T: GraphMutTxnT>(
    ...
    graph: &mut T::Graph,
    mut flag: EdgeFlags,
    mut k0: Vertex<NodeId>,
    mut k1: Vertex<NodeId>,
...
/// This is the unified registration function that handles all nodes uniformly.
pub fn register_node<T: GraphMutTxnT + DepsMutTxnT<DepsError = <T as GraphTxnT>::GraphError>>(
    txn: &mut T,
    internal: &NodeId,
    hash: &Hash,
    node_type: NodeType,
    ...
    debug!("registering node {:?} as {:?}", hash.to_base32(), node_type);
    let shash = hash.into();

    // Check if this node is already registered
    let already_registered = txn.get_node_type(internal)?.is_some();

    if !already_registered {
        // Only register the node if it's not already registered
        txn.put_external(internal, &shash)?;
        txn.put_internal(&shash, internal)?;
        txn.put_node_type(internal, node_type)?;
        debug!(
            "Registered new node {:?} as {:?}",
            hash.to_base32(),
            node_type
        );
    } else {
        debug!(
            "Node {:?} already registered, skipping node registration but wiring dependencies",
            hash.to_base32()
        );
    }

    // Always wire up dependencies
    for dep in dependencies.iter() {
        ...
        if let Some(dep_internal_ref) = txn.get_internal(&dep.into())? {
            ...
            if let Some(dep_node_type) = txn.get_node_type(&dep_internal)? {
                debug!(
                    "{:?} depends on {:?} (type: {:?})",
                    internal, dep_internal, dep_node_type
                );
            }
            ...
            debug!(
                "WARNING: {:?} has dependency {:?} without internal ID",
                internal, dep
            );
        }
    }
...
pub fn register_change<T: GraphMutTxnT + DepsMutTxnT<DepsError = <T as GraphTxnT>::GraphError>>(
    txn: &mut T,
    internal: &NodeId,
    hash: &Hash,
    change: &Change,
) -> Result<(), TxnErr<T::GraphError>> {
    register_node(txn, internal, hash, NodeType::Change, &change.dependencies)?;
...
/// Get a change or tag header based on the hash, automatically detecting the node type.
///
/// This is a convenience function that checks the node type in the database and calls
/// the appropriate ChangeStore method (`get_header` for changes, `get_tag_header` for tags).
///
/// # Arguments
/// * `txn` - Transaction with access to internal/external mappings and node types
/// * `changes` - ChangeStore implementation to retrieve the actual headers
/// * `hash` - The hash to look up (can be either a change hash or a tag merkle converted to hash)
///
/// # Returns
/// * `Ok(ChangeHeader)` - The header for the change or tag
/// * `Err` - If the hash is not found, the node type is unknown, or header retrieval fails
///
/// # Example
/// ```ignore
/// let header = get_header_by_hash(&txn, &repo.changes, &some_hash)?;
...
{
    // First, try to get the internal ID for this hash
    let shash = hash.into();
    if let Some(internal) = txn.get_internal(&shash)? {
        ...
        if let Some(node_type) = txn.get_node_type(&internal)? {
            ...
            // Node type not found - this might be a legacy node before node types were tracked
            debug!(
                "get_header_by_hash: node type not found for {}, assuming Change for backward compatibility",
                hash.to_base32()
            );
            return Ok(changes.get_header(hash)?);
        }
    ...
    // Hash not found in internal mappings - try as a change anyway for backward compatibility
    debug!(
        "get_header_by_hash: {} not found in internal mappings, trying as Change",
        hash.to_base32()
    );
