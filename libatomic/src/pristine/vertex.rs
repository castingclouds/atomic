...
impl Vertex<NodeId> {
    ...
    pub fn to_option(&self) -> Vertex<Option<NodeId>> {
        Vertex {
...
impl<H: Clone> Vertex<H> {
    ...
    /// [`Vertex<NodeId>`](struct.Vertex.html) as a
...
pub mod position_base32_serde {
    ...
    pub fn deserialize<'de, D: Deserializer<'de>>(d: D) -> Result<Position<NodeId>, D::Error> {
        d.deserialize_str(PositionDe {})
    }

    pub fn serialize<S: Serializer>(pos: &Position<NodeId>, s: S) -> Result<S::Ok, S::Error> {
        let b = pos.to_base32();
        s.serialize_str(&b)
    }
}
...
impl Position<NodeId> {
    pub fn inode_vertex(&self) -> Vertex<NodeId> {
        Vertex {
            change: self.change,
            start: self.pos,
            end: self.pos,
    ...
    pub(crate) const OPTION_ROOT: Position<Option<NodeId>> = Position {
        change: Some(NodeId::ROOT),
        pos: ChangePosition(L64(0u64)),
    };

    pub(crate) const BOTTOM: Position<NodeId> = Position {
        change: NodeId::ROOT,
