...
fn pending<T: libatomic::MutTxnTExt + libatomic::TxnT + Send + Sync + 'static>(
    txn: libatomic::ArcTxn<T>,
    channel: &libatomic::ChannelRef<T>,
    repo: &mut atomic_repository::Repository,
) -> Result<Option<libatomic::Hash>, anyhow::Error> {
    use libatomic::changestore::ChangeStore;

    let mut builder = libatomic::record::Builder::new();
    builder.record(
        txn.clone(),
    ...
    let mut pending_change = libatomic::change::Change::make_change(
        &*txn,
        channel,
        actions,
        contents,
        libatomic::change::ChangeHeader::default(),
        Vec::new(),
    )?;
    let (dependencies, extra_known) = libatomic::change::dependencies(
        &*txn,
        &*channel.read(),
...
#[cfg(unix)]
fn pager(repo_config_pager: Option<&atomic_config::Choice>) -> bool {
    if let Some(atomic_config::Choice::Never) = repo_config_pager {
        return false;
    } else if let Some(atomic_config::Choice::Never) = atomic_config::Global::load()
        .ok()
...
#[cfg(not(unix))]
fn pager(_repo_config_pager: Option<&atomic_config::Choice>) -> bool {
    false
}
...
/// a text editor.
fn make_changelist<S: libatomic::changestore::ChangeStore>(
    changes: &S,
    pullable: &[Node],
    verb: &str,
...
# be {}ed.\n",
    ...
    for p in pullable {
        ...
        first_p = false;
        let header = if p.is_change() {
            writeln!(v, "{}\n", p.hash.to_base32()).unwrap();
            let deps = changes.get_dependencies(&p.hash)?;
            if !deps.is_empty() {
            ...
            changes.get_header(&p.hash)?
        } else {
            writeln!(v, "{}\n", p.state.to_base32()).unwrap();
            changes.get_tag_header(&p.state)?
        };
...
fn parse_changelist(o: &[u8], states: &[Node]) -> Vec<Node> {
    use libatomic::Base32;
    let states: std::collections::HashSet<&Node> = states.iter().collect();
        ...
        o.lines()
            .filter_map(|l| {
                ::log::debug!(
                    "l = {:?} {:?}",
                    l,
                    libatomic::Merkle::from_base32(l.as_bytes())
                );
                let h_ = libatomic::Hash::from_base32(l.as_bytes())
                    .map(|h| Node::change(h, libatomic::Merkle::zero()));
                if let Some(h) = h_ {
                ...
                libatomic::Merkle::from_base32(&l.as_bytes()[..])
                    .map(|state| Node::tag(libatomic::Hash::NONE, state))
            })
            .collect()
    } else {
        Vec::new()
...
fn find_hash<B: libatomic::Base32>(
