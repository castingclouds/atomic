...
use libatomic::{MutTxnT, TxnT, TxnTExt};
    ...
    pub fn run(mut self) -> Result<(), anyhow::Error> {
        ...
        let (dependencies, extra_known) = {
            ...
            if self.tag {
                ...
                dependencies(&*txn_, &*channel.read(), change.changes.iter(), false)?
            }
        };
        ...
        if self.json {
            ...
            match change.write(
                ...
                Ok(()) => {}
                Err(libatomic::change::TextSerError::Io(e))
...
impl<W: termcolor::WriteColor> libatomic::change::WriteChangeLine for Colored<W> {
...
pub fn is_colored(repo_config_pager: Option<&atomic_config::Choice>) -> bool {
    let mut colors = atty::is(atty::Stream::Stdout);
    if let Ok((global, _)) = atomic_config::Global::load() {
        match global.colors {
            Some(atomic_config::Choice::Always) => colors = true,
            Some(atomic_config::Choice::Never) => colors = false,
            _ => {}
        }
        match global.pager {
            Some(atomic_config::Choice::Never) => colors = false,
