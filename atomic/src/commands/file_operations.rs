...
use libatomic::{MutTxnT, MutTxnTExt, TxnTExt};
use log::{debug, info};
...
impl Add {
    pub fn run(self) -> Result<(), anyhow::Error> {
        ...
        for path in self.paths.iter() {
            ...
            debug!("{:?}", meta);
            if !self.force
                && !libatomic::working_copy::filesystem::filter_ignore(
                    repo_path.as_ref(),
                    path.as_ref(),
                    meta.is_dir(),
                )
            ...
            if self.recursive {
                use libatomic::working_copy::filesystem::*;
                ...
                // If the user calls `atomic add .`, the entire path will be stripped, so we must account for this edge case
